...
// --------------------------------------------------------------------
// EDUNEXA PHASE 0: Pre-boot URI normalization (Double-slash compatibility)
// - Normalize internally (no redirects).
// - Collapse multiple slashes in PATH ONLY, preserve query string.
// --------------------------------------------------------------------
if (isset($_SERVER['REQUEST_URI'])) {
    $requestUri = $_SERVER['REQUEST_URI'];

    // Split path and query-string (preserve query as-is)
    ...
    if ($normalizedPath !== $path) {
        $_SERVER['REQUEST_URI'] = $normalizedPath . ($query !== '' ? '?' . $query : '');

        // Best-effort normalization for other server vars used by some environments
        if (isset($_SERVER['PATH_INFO'])) {
            $_SERVER['PATH_INFO'] = preg_replace('#/+#', '/', $_SERVER['PATH_INFO']);
        }

        if (isset($_SERVER['ORIG_PATH_INFO'])) {
            $_SERVER['ORIG_PATH_INFO'] = preg_replace('#/+#', '/', $_SERVER['ORIG_PATH_INFO']);
        }
