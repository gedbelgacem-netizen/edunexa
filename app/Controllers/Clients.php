...
class Clients extends Security_Controller {
    ...
    function index($tab = "") {
        // --------------------------------------------------------------------
        // EDUNEXA PHASE 0: Coordinator portal (client user) Learners UI
        // Allow client users to access /clients (read-only). Staff behavior unchanged.
        // --------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------
    // EDUNEXA PHASE 0: Learners Kanban data endpoint (HTML partial)
    // HARD LOCK: must return HTML partial (NOT JSON)
    // URL: /clients/all_clients_kanban_data
    // Supports two boards:
    // - status: new, onboarding, two_weeks_gap, dropout, inactive, extended
    // - sessions: sessions_today, sessions_this_week, sessions_last_week
    // Search-only: via "search" param (learner name or learner ID)
    // --------------------------------------------------------------------
    function all_clients_kanban_data() {
        // Staff: keep normal permission gate. Client: coordinator portal uses read-only view.
        ...
        if ($board === "sessions") {
            $columns = array(
                array("id" => "sessions_today", "title" => "Sessions Today"),
                array("id" => "sessions_this_week", "title" => "Sessions This Week"),
                array("id" => "sessions_last_week", "title" => "Sessions Last Week")
            );
        } else {
            // Default board: status
            $columns = array(
                array("id" => "new", "title" => "New"),
                array("id" => "onboarding", "title" => "Onboarding"),
                array("id" => "two_weeks_gap", "title" => "2 Weeks Gap"),
                array("id" => "dropout", "title" => "Dropout"),
                array("id" => "inactive", "title" => "Inactive"),
                array("id" => "extended", "title" => "Extended")
            );
        }
        ...
        $view_data = array(
            ...
            "columns" => $columns,
            // Phase 0: no DB wiring yet. Keep cards empty.
            "cards_by_column" => array()
        );
    ...
    function view($client_id = 0, $tab = "", $folder_id = 0) {
        ...
        // --------------------------------------------------------------------
        // EDUNEXA PHASE 0: Coordinator portal (client user) Learner view shells
        // Render lightweight shells with placeholders + Calendar tab (sessions read-only).
        // --------------------------------------------------------------------
        if ($this->login_user->user_type === "client") {
            $view_data = array(
            ...
            return $this->template->rander("clients/coordinator/view", $view_data);
        }
    ...
    // --------------------------------------------------------------------
    // EDUNEXA PHASE 0: Learner compact view shell
    // URL: /clients/compact_view/{id}
    // For client users: render coordinator learner compact shell.
    // For staff users: fallback to standard view() (Phase 0 minimal).
    // --------------------------------------------------------------------
    function compact_view($client_id = 0, $tab = "") {
        validate_numeric_value($client_id);
        ...
        if ($this->login_user->user_type === "client") {
            ...
            return $this->template->rander("clients/coordinator/compact_view", $view_data);
        }

        return $this->view($client_id, $tab);
    }
