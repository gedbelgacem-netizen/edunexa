...
class Signin extends App_Controller {
    ...
    function index() {
        if ($this->Users_model->login_user_id()) {
            // EDUNEXA PHASE 0 HOTFIX: user-type based landing (hard lock)
    ...
    function authenticate() {
        ...
        // --------------------------------------------------------------------
        // EDUNEXA PHASE 0 HOTFIX: Post-login landing (HARD LOCK)
        // - client => /dashboard
        // - staff  => /admin/sessions
        // NOTE: Do NOT refactor auth/session logic; only adjust redirect destination.
        // --------------------------------------------------------------------
        $user_id = $this->Users_model->login_user_id();
        $user = $this->Users_model->get_one($user_id);
