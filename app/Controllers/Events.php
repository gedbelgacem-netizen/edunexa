...
class Events extends Security_Controller {
    ...
    function index($encrypted_event_id = "") {
        ...
        $is_program_context = ($context === "program");

        if ($this->login_user->user_type === "staff") {
            $this->check_module_availability("module_event");
        } else {
            // EDUNEXA PHASE 0: allow Program Calendar for coordinators (client portal)
            // even if the legacy client-event permission is disabled.
            if (!$is_program_context && !$this->can_client_access("event")) {
    ...
    function calendar_events($filter_values = "", $event_label_id = 0, $client_id = 0) {
        ...
        // --------------------------------------------------------------------
        // EDUNEXA PHASE 0: Program Calendar (read-only, sessions-only)
        // Required response: JSON array ( [] is acceptable for Phase 0 )
        ...
        // Avoid PHP notices if called without start/end params
        if (!$start) {
            $start = get_my_local_time("Y-m-d");
        }
        if (!$end) {
            $end = $start;
        }
    ...
    //view an event (modal fragment)
    function view($id = "") {
        $context = $this->request->getGet("context");

        // --------------------------------------------------------------------
        // EDUNEXA PHASE 0: Program Calendar session detail modal (stub)
        // Response shape: HTML fragment only (ajaxModal)
        ...
        $encrypted_event_id = $id ? $id : $this->request->getPost('id');
        ...
        // When called via modal_anchor, encrypted_event_id is usually posted.
        // Phase 0 supports /events/view/{id} as well (id in URI).
        if (!$id) {
            $this->validate_submitted_data(array(
                "id" => "required"
            ));
