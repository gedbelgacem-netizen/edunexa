...
class Left_menu {
    ...
    function rander_left_menu($is_preview = false, $type = "default") {
        // --------------------------------------------------------------------
        // EDUNEXA PHASE 0 HOTFIX: Sidebar navigation lock
        // Apply the Phase 0 locked menu for BOTH faces (client + staff).
        // Do NOT rely on DB-stored menus in Phase 0.
        // --------------------------------------------------------------------
        if (!$is_preview && in_array($this->ci->login_user->user_type, array("client", "staff"), true)) {
            $view_data = array();
            $view_data["sidebar_menu"] = $this->_get_edunexa_coordinator_menu_items();
    ...
    // --------------------------------------------------------------------
    // EDUNEXA PHASE 0: Fixed Phase 0 left menu definition (client + staff)
    // --------------------------------------------------------------------
    private function _get_edunexa_coordinator_menu_items() {
        ...
        $settings_profile_url = ($this->ci->login_user->user_type === "client")
            ? get_uri("clients/contact_profile/" . $contact_id)
            : get_uri("team_members/view/" . $contact_id);
        ...
        return array(
            ...
            array(
                "name" => "Program",
                "url" => get_uri("events?context=program"),
                "class" => "book-open",
                "is_custom_menu_item" => true,
                "submenu" => array(
                    array(
